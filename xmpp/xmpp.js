...
if (config.bEnableHTTPS) {
    ...
    log.xmpp(`XMPP- Matchmaker are now listening by selected port ${port} `);
}
...
app.get("/status", (req, res) => {
    ...
    const matchmakerModule = require("../matchmaker/matchmaker.js");
    const queueStatus = {};
    if (matchmakerModule.queue) {
        for (const [playlist, players] of matchmakerModule.queue) {
            queueStatus[playlist] = players.length;
        }
    }

    res.json({
        "onlinePlayers": global.Clients ? global.Clients.length : 0,
        "queuedPlayers": Object.values(queueStatus).reduce((a, b) => a + b, 0),
        "queueByPlaylist": queueStatus,
        "activeMatches": matchmakerModule.activeSessions ? matchmakerModule.activeSessions.size : 0
    });
});
