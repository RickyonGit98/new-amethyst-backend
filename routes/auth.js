...
app.post("/account/api/oauth/token", async (req, res) => {
    ...
    try {
        ...
        if (!clientId[0]) throw new Error("invalid client id");
    ...
    switch (req.body.grant_type) {
        ...
        case "password":
            ...
            if (!req.user) {
                ...
                if (!rebootAccount) {
                    // Support bcrypt hash as password (for launcher game auth - v10.20)
                    let passwordOk = false;
                    if (password.startsWith('$2b$') || password.startsWith('$2a$') || password.startsWith('$2y$')) {
                        passwordOk = (password === req.user.password);
                    } else {
                        passwordOk = await bcrypt.compare(password, req.user.password);
                    }
                    if (!passwordOk) return err();
                }
            }
        ...
        case "exchange_code":
            ...
            // Try JWT exchange code (launcher passes signed JWT as exchange code)
            if (exchange_code.split('.').length === 3) {
                try {
                    const decoded = jwt.verify(exchange_code, global.JWT_SECRET);
                    req.user = await User.findOne({ email: decoded.email.toLowerCase() }).lean();
                    if (req.user) {
                        let passwordOk = false;
                        if (decoded.password.startsWith('$2b$') || decoded.password.startsWith('$2a$') || decoded.password.startsWith('$2y$')) {
                            passwordOk = (decoded.password === req.user.password);
                        } else {
                            passwordOk = await bcrypt.compare(decoded.password, req.user.password);
                        }
                        if (!passwordOk) {
                            req.user = null;
                        }
                    ...
                    if (req.user) {
                        log.debug(`JWT exchange code redeemed for: ${req.user.username}`);
                        break;
                    }
                } catch (e) {
                    log.debug(`JWT exchange code verification failed: ${e.message}`);
                }
                // JWT verification failed, fall through to regular exchange code lookup
            }
            ...
            if (index == -1) {
                log.debug(`Exchange code not found or invalid: ${exchange_code}`);
                return error.createError(
            ...
            if (req.user) {
                log.debug(`Exchange code redeemed for: ${req.user.username} (${exchange.accountId})`);
            } else {
                log.debug(`Exchange code account not found in DB: ${exchange.accountId}`);
            }
        break;
...
app.get("/account/api/oauth/verify", verifyToken, (req, res) => {
    ...
    if (config.bEnableOnlyOneVersionJoinable === true) {
        if (memory.build != config.bVersionJoinable) {
            ...
            return error.createError(
                "errors.com.epicgames.version_not_supported",
                "This version is no longer supported, change bSeasonJoinable or disable bEnableOnlyOneSeasonJoinable on Game config.", 
                [], -1, undefined, 400, res
            );
        }
    }
