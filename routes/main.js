...
app.post("/fortnite/api/game/v2/chat/*/*/*/pc", (req, res) => {
    ...
    let resp = config.chat.EnableGlobalChat ? { "GlobalChatRooms": [{ "roomName": "titybackendglobal" }] } : {};
...
app.get("/launcher/api/public/assets/*", async (req, res) => {
    res.json({
        ...
        "items": {
            "MANIFEST": {
                "signature": "Game",
                "distribution": "https://titybackend.ol.epicgames.com/",
                "path": "Builds/Fortnite/Content/CloudDir/tityBackend.manifest",
...
app.get("/Builds/Fortnite/Content/CloudDir/*.manifest", async (req, res) => {
    ...
    const manifest = fs.readFileSync(path.join(__dirname, "..", "responses", "CloudDir", "TityBackend.manifest"));
...
app.get("/Builds/Fortnite/Content/CloudDir/*.chunk", async (req, res) => {
    ...
    const chunk = fs.readFileSync(path.join(__dirname, "..", "responses", "CloudDir", "TityBackend.chunk"));
