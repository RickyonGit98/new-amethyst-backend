...
function getContentPages(req) {
    ...
    try {
        ...
        contentpages.subgameselectdata.battleRoyale.message.title = "Experience Chapter 2 Season 2 with Amethyst";
        contentpages.subgameselectdata.battleRoyale.message.body = "discord.gg/amethyst";
    } catch {}
...
function getItemShop() {
    ...
    try {
        for (let value in CatalogConfig) {
            ...
            let i = catalog.storefronts.findIndex(p => p.name == (value.toLowerCase().startsWith("daily") ? "Amethyst shop" : "Thanks for playing!"));
            if (i == -1) continue;
...
async function createSAC(code, username, creator) {
    ...
    if (account == null) return { message: `**${username}** doesnt exist!`, status: 400 }
